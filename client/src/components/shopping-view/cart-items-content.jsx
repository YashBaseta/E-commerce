...
function UserCartItemsContent({cartItem}) {
    ...
    <div className="flex-1">
        ...
        <div className="flex items-center mt-1 gap-2">
            <Button onClick={() => handleUpdateQuantity(cartItem,'minus')} disabled={cartItem?.quantity === 1} className="h-6 w-6 rounded-full" varient="outline" size="icon">
                <Minus className="w-4 h-4"/>

                <span className="sr-only">Decrease</span>
            </Button>
            <span className="font-semibold">{cartItem?.quantity}</span>
            <Button onClick={() => handleUpdateQuantity(cartItem,'plus')} className="h-6 w-6 rounded-full" varient="outline" size="icon">
